  ...
  if (splitSection) {
    ...
    if (item) {
      ...
      if (assets.splitImage) {
        item.Image = [imageBlok(assets.splitImage)];
      }
...
function patchDisclaimer(disclaimerRichtext, amount, days) {
  ...
  function patchNode(node) {
    if (node.type === 'text' && typeof node.text === 'string') {
      // Replace dollar amount occurrences (e.g. $200,000.00, $400,000.00)
      node.text = node.text.replace(/\$[\d,]+\.\d{2}\s*USD/g, `${amount} USD`);
      // Replace "ninety (90)" or any "word (number)" pattern for days
      node.text = node.text.replace(
        /[a-z]+ \(\d+\) days/gi,
        `${daysWord} (${daysInt}) days`,
      );
      // Replace bare "90-day" style references
      node.text = node.text.replace(/\d+-day/g, `${daysInt}-day`);
    }
