...
export function buildPartnerStoryContent(template, inputs, assets) {
  ...
  if (heroSection) {
    ...
    if (textHeader) {
      ...
      // Eyebrow icon — asset is uploaded to Storyblok for manual assignment,
      // but not written to the story component (user assigns it in Storyblok).
...
function patchDisclaimer(disclaimerRichtext, amount, days) {
  ...
  const daysWord = toWords(daysInt);
